...
#####################################################
class _cRBF():
    def __init__(self, x, y, eps=None, method='GUASSIAN', periodic=False):
        mL = ['GUASSIAN', 'MULTIQUADRATIC', 'INVERSE']
        if method.upper() not in mL:
            raise NameError('Available methods: ["guassian", "multiquadratic", "inverse"]')
        else:
            self.method = method.upper()
        if len(x) != len(y):
            raise ValueError('len(x) != len(y)')
        else:
            self.x = _np.array(x, dtype='float32')
            self.y = _np.array(y, dtype='float32')
        if eps is None:
            self.eps = _np.mean(_np.diff(x))
        else:
            self.eps = eps
        self.periodic = periodic
        wgtMat = _np.zeros([len(x), len(x)], dtype='float32')
        for i in range(len(self.x)):
            wgtMat[i] = _np.abs(self.x - self.x[i])
        self.weight = _np.matmul(_np.linalg.pinv(self._kernel(wgtMat)), self.y)

    def _kernel(self, mat):
        if self.periodic == True:
            mat = _np.deg2rad(mat)
            eps = _np.deg2rad(self.eps)
            arg = (_np.sin(mat / 2) / eps) ** 2
        else:
            eps = self.eps
            arg = (mat / eps) ** 2
        if self.method == 'GUASSIAN':
            return _np.exp(-1 * arg)
        elif self.method == 'MULTIQUADRATIC':
            return _np.sqrt(1 + arg)
        elif self.method == 'INVERSE':
            return 1 / (1 + arg)

    def interp(self, xx):
        lenx = len(self.x)
        interpMat = _np.zeros([len(xx), lenx], dtype='float')
        for i in range(lenx):
            interpMat[:, i] = _np.abs(xx - self.x[i])
        return _np.matmul(self._kernel(interpMat), self.weight)
...
class _cMAP:
    def __init__(self, inFile):
        ...
        for line in mapFile:
            if 'AATRAJ' in line.split('#')[0]:
                ...
                if lsplit0.upper() == 'MOL':
                    ...
                    self.nMOL = _np.array(lsplit1.split(), dtype=_np.int32)
        ...
        self.nATOM = _np.zeros(len(self.MOL), dtype=_np.int32)
        ...
        self.nBEAD = _np.zeros(len(self.MOL), dtype=_np.int32)
        self.BONDterm = _np.zeros(len(self.MOL), dtype=bool)
        self.BONDtype = _np.zeros(len(self.MOL), dtype=object)
        self.nBONDtype = _np.zeros(len(self.MOL), dtype=_np.int32)
        ...
        self.nANGLEtype = _np.zeros(len(self.MOL), dtype=_np.int32)
        ...
        self.nDIHEDRALtype = _np.zeros(len(self.MOL), dtype=_np.int32)
            ...
            while line < len(mapFile):
                ...
                if len(lsplit0) == 3:
                    ...
                    if lsplit0[0] == 'MOL' and lsplit0[1] == self.MOL[mol] and lsplit0[2] == 'ATOM':
                        ...
                        while n < nbead:
                            ...
                            if 'MOL' in lco:
                                ...
                                try:
                                    ...
                                    bead_map[n] = _np.array(lco.split(':')[1].split(), dtype=_np.int32)
                                    n += 1
                                except:
                                    pass
                                line += 1
                        ...
                        n = 0
                        line += 1
                        bond_type = _np.zeros(nbond_type, dtype=_np.int32)
                        ...
                        while n < nbond_type:
                            ...
                            if 'MOL' in lco:
                                ...
                                try:
                                    ...
                                    bond_type_idx[n] = _np.array(temp, dtype=_np.int32)
                                    n += 1
                                except:
                                    pass
                                line += 1
    ...
    def do_classification(self):
        ...
        TotAnglesTypeClass = _np.zeros([0, 2], dtype=_np.int32)

        TotDihedral = _np.zeros(len(self.MOL), dtype=object)
        TotDihedralType = _np.zeros(len(self.MOL), dtype=object)
        TotDihedralTypeClass = _np.zeros([0, 3], dtype=_np.int32)
        ...
        for mol in range(len(self.MOL)):
            ...
            allBOND = _np.zeros([nAllBond, 2], dtype=_np.int32)
            n = 0
            for i in range(self.nBONDtype[mol]):
            ...
            for bead in range(self.nBEAD[mol]):
                maplist = _np.array([], dtype=_np.int32)
            ...
            if self.ANGLEterm[mol]:
                ...
                allangle = _np.zeros([m, 3], dtype=_np.int32)
                n = 0
                ...
                allangleType = _np.zeros([len(TotAngles[mol]), 2], dtype=_np.int32)
                ...
                allangleTypeClass = _np.zeros([m, 2], dtype=_np.int32)
                n = 0
                for i in range(m):
            ...
            if self.DIHEDRALterm[mol]:
                ...
                alldihedral = _np.zeros([m, 4], dtype=_np.int32)
                n = 0
                for i in range(self.nBEAD[mol]):
                    for j in BONDtree[i]:
                        if j != i:
                ...
                alldihedralType = _np.zeros([len(TotDihedral[mol]), 3], dtype=_np.int32)
                ...
                dihedralTypeClass = _np.zeros([m, 3], dtype=_np.int32)
                n = 0
                for i in range(m):
        ...
        lmpIdSeq = _np.zeros(len(typeSeq), dtype=_np.int32)
        typeGroup = []
        ...
        temp = _np.zeros([m, 2], dtype=_np.int32)
        n = 0
        for i in range(m):
        ...
        temp = _np.zeros([m, 3], dtype=_np.int32)
        n = 0
        for i in range(m):
        ...
        for mol in range(len(self.MOL)):
            ...
            dihedraltype = _np.zeros(self.nDIHEDRALtype[mol], dtype=_np.int32)
            ...
            if self.nDIHEDRALtype[mol] > 0:
                ...
                for i in range(len(TotDihedralTypeClass)):
                    IDsTemp = _np.zeros([0, 4], dtype=_np.int32)
...
#####################################################
class _cTRAJ:
    def __init__(self, inFile, mode, nAtoms=None):
        inFile = str(inFile)
        self._mode = str(mode)
        if inFile.endswith('.xtc'):
            self.Type = 'XTC'
        elif inFile.endswith('.trr'):
            self.Type = 'TRR'
            ...
            raise _scg4pyError('supported trajectory file formats: ".xtc", ".trr", ".dcd", ".gro", ".pdb", ".lammpstrj"')
        if self._mode == 'r':
            if self.Type == 'XTC' or self.Type == 'TRR' or self.Type == 'DCD':
                self._fid = _MDA.Universe(inFile)
            else:
                self._fid = open(inFile, mode='r')
        elif self._mode == 'w':
            if self.Type == 'XTC' or self.Type == 'TRR'  or self.Type == 'DCD':
                self._fid = _MDA.Writer(inFile, n_atoms=nAtoms)
            else:
    ...
    def _setNone(self):
        self.time = self.timeStep = self.dt = self.nAtom = self.atId = self.atName = self.atType = None
        self.resId = self.resName = self.x = self.y = self.z = self.xs = self.ys = self.zs = None
        self.xu = self.yu = self.zu = self.ix = self.iy = self.iz = self.fx = self.fy = self.fz = None
        self.lmpBox = self.boxMat = self.boxCryst = None
        self.eof = False

    def close(self):
        if self.Type == 'XTC' or self.Type == 'DCD' or self.Type == 'TRR':
            if self._mode == 'r':
                self._fid.trajectory.close()
            else:
                self._fid.close()
        else:
            self._fid.close()
    ...
    def read(self, i=None):
        ...
        if self.Type == 'GRO':
            ...
            self.readPDBsnap()
        elif self.Type == 'XTC' or self.Type == 'TRR' or self.Type == 'DCD':
            self.readMDAsnap(i)
        elif self.Type == 'LMP':
            self.readLMPsnap()
    ...
    def writeGROsnap(self):
        if self.atId is None:
            atId = _np.arange(1, self.nAtom + 1)
        else:
            atId = self.resId
        if self.resId is None:
            resId = _np.arange(1, self.nAtom + 1)
        else:
            resId = self.resId
        if self.resName is None:
            resName = _np.array(['UNK'] * self.nAtom, '<U5')
        else:
            resName = self.resName
        if self.atName is None:
            atName = _np.array(['X'] * self.nAtom, '<U5')
        else:
            atName = self.resName
        self._fid.write('Generated by SCG4PY program: t= {}\n'.format(self.time))
        self._fid.write('{:6d}\n'.format(self.nAtom))
        for i in _np.arange(len(atId)):
            newResId = int(resId[i] % 1e5)
            newAtId = int(atId[i] % 1e5)
            self._fid.write('{0:5d}{1:<5s}{2:^5s}{3:5d}'.format(newResId, resName[i], atName[i], newAtId))
    ...
    def readPDBsnap(self):
        try:
            ...
            if any([i == 0 for i in self.boxMat]):
                print('warning: not found "CRYST1" information')
            self.resId = _np.zeros(0, dtype=_np.int32)
            self.resName = _np.zeros(0, dtype='U5')
            self.atName = _np.zeros(0, dtype='U5')
            self.atId = _np.zeros(0, dtype=_np.int32)
            self.x = _np.zeros(0, dtype=_np.float32)
            self.y = _np.zeros(0, dtype=_np.float32)
            self.z = _np.zeros(0, dtype=_np.float32)
            i = 0
            while ('ENDMDL' not in line) and ('END' not in line):
                if ('ATOM' in line) or ('HETATM' in line):
                    self.atId = _np.append(self.atId, i + 1)
                    self.atName = _np.append(self.atName, line[12:16].strip())
                    self.resName = _np.append(self.resName, line[17:21].strip())
                    if i == 0:
                        self.resId = _np.append(self.resId, 1)
                        prevId = int(line[22:26])
                    else:
                        if int(line[22:26]) == prevId:
                            self.resId = _np.append(self.resId, self.resId[-1])
                        else:
                            self.resId = _np.append(self.resId, self.resId[-1] + 1)
                            prevId = int(line[22:26])
                    self.x = _np.append(self.x, float(line[30:38]))
                    self.y = _np.append(self.y, float(line[38:46]))
                    self.z = _np.append(self.z, float(line[46:54]))
                    i += 1
                line = self._fid.readline()
                if self._fid.tell() != ftell:
                    ftell = self._fid.tell()
    ...
    def writePDBsnap(self, modelN=None, sysName=None):
        ...
        for i in _np.arange(self.nAtom):
            self._fid.write('{0:6s}{1:5d} {2:^4s} {3:<4s} {4:4d}    '.format(
                'ATOM', int(atId[i] % 1e5), atName[i], resName[i], int(resId[i] % 1e4)))
            self._fid.write(
                '{0:8.3f}{1:8.3f}{2:8.3f}                       C\n'.format(self.x[i], self.y[i], self.z[i]))
        self._fid.write('TER\nENDMDL\n')

    def readMDAsnap(self, i):
        if i == len(self._fid.trajectory):
            self.eof = True
        else:
            try:
                snap = self._fid.trajectory[i]
                if snap.has_positions:
                    self.x = snap.positions[:, 0]
                    self.y = snap.positions[:, 1]
                    self.z = snap.positions[:, 2]
                if snap.has_forces:
                    self.fx = snap.forces[:, 0] / 4.184
                    self.fy = snap.forces[:, 1] / 4.184
                    self.fz = snap.forces[:, 2] / 4.184
                self.nAtom = snap.n_atoms
                self.time = snap.data['time']
                self.timeStep = snap.data['step']
                self.dt = snap.data['dt']
                lx = snap.triclinic_dimensions[0][0]
                ly = snap.triclinic_dimensions[1][1]
                lz = snap.triclinic_dimensions[2][2]
                xy = snap.triclinic_dimensions[1][0]
                xz = snap.triclinic_dimensions[2][0]
                yz = snap.triclinic_dimensions[2][1]
                self.boxMat = _np.array([lx, ly, lz, xy, xz, yz], dtype=_np.float32)
                self._box_mat2cryst()
                self.atId = _np.arange(1, self.nAtom + 1)
            except:
                print('\n\nError in parsing snapshot {}'.format(i))
                self.eof = True

    def writeMDAsnap(self):
        lx, ly, lz, xy, xz, yz = self.boxMat[0], self.boxMat[1], self.boxMat[2], \
                                 self.boxMat[3], self.boxMat[4], self.boxMat[5]
        # self._box_mat2cryst()
        snap = _MDA.Universe.empty(n_atoms=self.nAtom, trajectory=True)
        ts = snap.trajectory[0]
        data = {'time': 0, 'step': 0, 'dt': 10.0}
        if self.time is not None:
            ts.time = self.time
            data['time'] = self.time
        if self.timeStep is not None:
            data['step'] = self.timeStep
        if self.dt is not None:
            data['dt'] = self.dt
        ts.data = data
        xyz = _np.zeros([self.nAtom, 3], dtype=_np.float32)
        xyz[:, 0] = self.x
        xyz[:, 1] = self.y
        xyz[:, 2] = self.z
        tric_mat = _np.zeros([3, 3], dtype=_np.float32)
        tric_mat[0][0] = lx
        tric_mat[1][1] = ly
        tric_mat[2][2] = lz
        tric_mat[1][0] = xy
        tric_mat[2][0] = xz
        tric_mat[2][1] = yz
        ts.triclinic_dimensions = tric_mat
        # ts.dimensions = self.boxCryst
        ts.positions = xyz
        self._fid.write(snap)

    def readLMPsnap(self):
        try:
            self._fid.readline()
    ...
    def unscaleSnap(self):
        ...
        if cond:
            ...
            mess = 'Unscaling error: scaled coordinates not found.'
    ...
    def scaleSnap(self):
        ...
        if cond1:
            ...
            mess = 'Scaling error: unscaled coordinates not found.'
            raise _scg4pyError(mess)
        if self.lmpBox is None:
            xlo, ylo, zlo = 0.0, 0.0, 0.0
        lx, ly, lz, xy, xz, yz = self.boxMat[0], self.boxMat[1], self.boxMat[2], \
    ...
    def unwrapLMPsnap(self):
        ...
        if cond:
            ...
            mess = 'Unwrapping error: "xs", "ys", "zs", "ix", "iy", "iz" not found.'
            raise _scg4pyError(mess)
        if self.lmpBox is None:
            xlo, ylo, zlo = 0.0, 0.0, 0.0
        lx, ly, lz, xy, xz, yz = self.boxMat[0], self.boxMat[1], self.boxMat[2], \
    ...
    def wrapLMPsnap(self):
        if (self.xu is None) or (self.yu is None) or (self.zu is None):
            mess = 'Wrapping error: "xu", "yu", "zu" not found.'
            raise _scg4pyError(mess)
        self.scaleSnap()
    ...
    def SetNonBondedIndex(self):
        ...
        for i in range(len(self.BEADtypeSet)):
            for j in range(i, len(self.BEADtypeSet)):
                ...
                if i == j:
                    itC = _iter.combinations(indI, 2)
                    pairs = _np.array(list(itC), dtype=int)
                else:
...
class _cTAB():
    def __init__(self, inFile, mode):
        ...
        if inFile.endswith('.hist'):
            ...
            self.Type = 'dPOT'
        elif inFile.endswith('.hlog'):
            self.Type = 'hLOG'
        elif inFile.endswith('.force'):
            self.Type = 'FORCE'
        else:
            raise IOError('input file should be one of: ".hist", ".dist", ".pot", ".dpot"')
        if self._mode == 'r':
            ...
            self.BondType = _np.zeros(nBtab, dtype='int32')
            self.BondBinW = _np.zeros(nBtab, dtype='float32')
            self.AngleName = _np.zeros([nAtab, 3], dtype='U5')
            self.AngleType = _np.zeros(nAtab, dtype='int32')
            self.AngleBinW = _np.zeros(nAtab, dtype='float32')
            self.DihedralName = _np.zeros([nDtab, 4], dtype='U5')
            self.DihedralType = _np.zeros(nDtab, dtype='int32')
            self.DihedralBinW = _np.zeros(nDtab, dtype='float32')
            self.NonBondType = _np.zeros([nNBtab, 2], dtype='U5')
            self.NonBondBinW = _np.zeros(nNBtab, dtype='float32')
            if self.Type == 'hLOG':
                self.BondMinMax = _np.zeros([nBtab, 2], dtype='float32')
                self.AngleMinMax = _np.zeros([nAtab, 2], dtype='float32')
                self.DihedralMinMax = _np.zeros([nDtab, 2], dtype='float32')
                self.NonBondMinMax = _np.zeros([nNBtab, 2], dtype='float32')
            else:
            ...
            if self.Type == 'FORCE':
                self.BondStd = _np.zeros(nBtab, dtype=object)
                self.AngleStd = _np.zeros(nAtab, dtype=object)
                self.DihedralStd = _np.zeros(nDtab, dtype=object)
                self.NonBondStd = _np.zeros(nNBtab, dtype=object)
            line = 0
            for i in range(nNBtab):
                ...
                line += 1
                if self.Type == 'hLOG':
                    minD = float(tabL[line].split('=')[1])
                    line += 1
                    maxD = float(tabL[line].split('=')[1])
                    self.NonBondMinMax[i] = [minD, maxD]
                    line += 1
                    self.NonBondBinW[i] = float(tabL[line].split('=')[1])
                elif self.Type == 'FORCE':
                    self.NonBondX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.NonBondBinW[i] = _np.mean(_np.diff(self.NonBondX[i]))
                    line += 1
                    self.NonBondY[i] = _np.array(tabL[line].split(), dtype='float32')
                    line += 1
                    self.NonBondStd[i] = _np.array(tabL[line].split(), dtype='float32')
                else:
                    self.NonBondX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.NonBondBinW[i] = _np.mean(_np.diff(self.NonBondX[i]))
                    line += 1
            ...
            for i in range(nBtab):
                ...
                if self.Type == 'hLOG':
                    ...
                    line += 1
                    maxD = float(tabL[line].split('=')[1])
                    self.BondMinMax[i] = [minD, maxD]
                    line += 1
                    self.BondBinW[i] = float(tabL[line].split('=')[1])
                elif self.Type == 'FORCE':
                    self.BondX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.BondBinW[i] = _np.mean(_np.diff(self.BondX[i]))
                    line += 1
                    self.BondY[i] = _np.array(tabL[line].split(), dtype='float32')
                    line += 1
                    self.BondStd[i] = _np.array(tabL[line].split(), dtype='float32')
                else:
                    self.BondX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.BondBinW[i] = _np.mean(_np.diff(self.BondX[i]))
                    line += 1
            ...
            for i in range(nAtab):
                ...
                line += 1
                if self.Type == 'hLOG':
                    minA = float(tabL[line].split('=')[1])
                    line += 1
                    maxA = float(tabL[line].split('=')[1])
                    self.AngleMinMax[i] = [minA, maxA]
                    line += 1
                    self.AngleBinW[i] = float(tabL[line].split('=')[1])
                elif self.Type == 'FORCE':
                    self.AngleX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.AngleBinW[i] = _np.mean(_np.diff(self.AngleX[i]))
                    line += 1
                    self.AngleY[i] = _np.array(tabL[line].split(), dtype='float32')
                    line += 1
                    self.AngleStd[i] = _np.array(tabL[line].split(), dtype='float32')
                else:
                    self.AngleX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.AngleBinW[i] = _np.mean(_np.diff(self.AngleX[i]))
                    line += 1
            ...
            for i in range(nDtab):
                ...
                if self.Type == 'hLOG':
                    ...
                    line += 1
                    maxA = float(tabL[line].split('=')[1])
                    self.DihedralMinMax[i] = [minA, maxA]
                    line += 1
                    self.DihedralBinW[i] = float(tabL[line].split('=')[1])
                elif self.Type == 'FORCE':
                    self.DihedralX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.DihedralBinW[i] = _np.mean(_np.diff(self.DihedralX[i]))
                    line += 1
                    self.DihedralY[i] = _np.array(tabL[line].split(), dtype='float32')
                    line += 1
                    self.DihedralStd[i] = _np.array(tabL[line].split(), dtype='float32')
                else:
                    self.DihedralX[i] = _np.array(tabL[line].split(), dtype='float32')
                    self.DihedralBinW[i] = _np.mean(_np.diff(self.DihedralX[i]))
                    line += 1
            ...
            self.BondStd = None
            self.AngleStd = None
            self.DihedralStd = None
            self.NonBondStd = None
            self._fileName = inFile
        else:
            raise IOError('opening mode should be one of "r" or "w".')

    def setattr(self, NonBondType, NonBondX, NonBondY, NonBondStd=None,
                BondName=None, BondType=None, BondX=None, BondY=None, BondStd=None,
                AngleName=None, AngleType=None, AngleX=None, AngleY=None, AngleStd=None,
                DihedralName=None, DihedralType=None, DihedralX=None, DihedralY=None, DihedralStd=None):
        if self._mode == 'w':
            self.BondName = BondName
            self.BondType = BondType
            self.BondX = BondX
            self.BondY = BondY
            self.BondStd = BondStd
    ...
    def smooth(self, x, y, sig):
        if sig > 0:
            ...
            yy = _np.zeros(_np.size(x), dtype='float32')
    ...
    def write(self):
        if self._mode == 'w':
            ...
            for i in range(len(self.NonBondType)):
                ...
                if self.NonBondStd is not None:
                    self._fid.write('\n')
                    for j in range(len(self.NonBondStd[i])):
                        self._fid.write('{:.12e} '.format(self.NonBondStd[i][j]))
                self._fid.write('\n\n')
            if self.BondType is not None:
                for i in range(len(self.BondType)):
                    ...
                    if self.BondStd is not None:
                        self._fid.write('\n')
                        for j in range(len(self.BondStd[i])):
                            self._fid.write('{:.12e} '.format(self.BondStd[i][j]))
                    self._fid.write('\n\n')
            if self.AngleType is not None:
                for i in range(len(self.AngleType)):
                    ...
                    if self.AngleStd is not None:
                        self._fid.write('\n')
                        for j in range(len(self.AngleStd[i])):
                            self._fid.write('{:.12e} '.format(self.AngleStd[i][j]))
                    self._fid.write('\n\n')
            if self.DihedralType is not None:
                for i in range(len(self.DihedralType)):
                    ...
                    if self.DihedralStd is not None:
                        self._fid.write('\n')
                        for j in range(len(self.DihedralStd[i])):
                            self._fid.write('{:.12e} '.format(self.DihedralStd[i][j]))
                    self._fid.write('\n\n')
            self._fid.close()
    ...
    def extrpPot_Bond(self, i, binW, maxF, cutoff):
        ...
        m2 = fC[0]
        y2 = yC[0]
        mat = _np.array([[2 * x1, 1, 0], [2 * x2, 1, 0], [x2 ** 2, x2, 1]], dtype='float32')
    ...
    def extrpPot_Angle(self, i, binW, maxF):
        ...
        y1 = yC[-1]
        x1 = xC[-1]
        mat = _np.array([[x1 ** 2, x1, 1], [2 * x1, 1, 0], [2 * x2, 1, 0]], dtype='float32')
    ...
    def extrpPot_Dihedral(self, i, binW):
        ...
        # 2- Pot interpolation
        # Rbf = _cRBF(x_ext, y_ext, periodic=True)
        # Pot = Rbf.interp(X)
    ...
    def extrpPot_NonBonded(self, i, binW, maxF, cutoff, skin):
        ...
        mat = _np.array([[x1 ** 5, x1 ** 3, x1, 1], [x2 ** 5, x2 ** 3, x2, 1],
                         [5 * x1 ** 4, 3 * x1 ** 2, 1, 0], [5 * x2 ** 4, 3 * x2 ** 2, 1, 0]], dtype='float32')
...
class _cDFs():
    ...
    def calcA(self, x, y, z, setIdx):
        ...
        for i in range(len(setIdx)):
            ...
            BA = _np.zeros([len(xB), 3])
            BA[:, 0] = xA - xB
            BA[:, 1] = yA - yB
            BA[:, 2] = zA - zB
            BC = _np.zeros([len(xB), 3])
            BC[:, 0] = xC - xB
            BC[:, 1] = yC - yB
            BC[:, 2] = zC - zB

            ba = _np.linalg.norm(BA, axis=1)
            bc = _np.linalg.norm(BC, axis=1)
            ctheta = _np.sum(BA * BC, axis=1) / (ba * bc)
            ctheta[ctheta > 1.0] = 1.0
            ctheta[ctheta < -1.0] = -1.0
            AngleData[i] = _np.rad2deg(_np.arccos(ctheta))
        return AngleData
    ...
    def calcD(self, x, y, z, setIdx):
        ...
        for i in range(len(setIdx)):
            ...
            BC[:, 2] = zC - zB
            CB = -1 * BC
            CD = _np.zeros([len(xB), 3])
            CD[:, 0] = xD - xC
            CD[:, 1] = yD - yC
            CD[:, 2] = zD - zC
            ab = _np.linalg.norm(AB, axis=1)
            bc = _np.linalg.norm(BC, axis=1)
            cd = _np.linalg.norm(CD, axis=1)

            ABxBC = _np.cross(AB, BC)
            abxbc = _np.linalg.norm(ABxBC, axis=1)
            n1 = ABxBC / abxbc.repeat(3).reshape([len(abxbc), 3])
            BCxCD = _np.cross(BC, CD)
            bcxcd = _np.linalg.norm(BCxCD, axis=1)
            n2 = BCxCD / bcxcd.repeat(3).reshape([len(bcxcd), 3])
            nBC = BC / bc.repeat(3).reshape([len(bc), 3])
            u = _np.cross(nBC, n2)
            n1_u = _np.sum(n1 * u, axis=1)
            n1_n2 = _np.sum(n1 * n2, axis=1)
            phi = -1 * _np.arctan2(n1_u, n1_n2)
...
• The CG structure of each molecule type in different files (*.pdb).'''
    ...
    if readTRJ:
        ...
        AAtraj.read(i=0)
        CGtraj = _cTRAJ(sysMap.CGTRAJ, 'w', nAtoms=len(allBEADmap))
        ...
        while True:
            if n % 2 == 0:
                if AAtraj.time:
                    ...
                    print('\rSnapshot = {}'.format(n + 1), end='')
            if n == 0:
                print()
            AAtraj.dt = CGtraj.dt
...
def histCalc(inTraj, top, outTab=None, RMaxNB=20, RMaxB=10,
             BinNB=0.2, BinB=0.02, BinA=2, BinD=5, normalizeBonded=False, start=1, stop=-1):
...
• System’s distribution functions ([outTab].hist if normalizeBonded=False or [outTab].dist if normalizeBonded=True)'''
    ...
    if stop == -1:
        ...
        raise ValueError('"stop > start" or' "stop = -1")
    ...
    minR = _np.ones(nNB, dtype=float) * 1e10
    maxR = _np.ones(nNB, dtype=float) * -1e10
    ...
    minB = _np.ones(nB, dtype=float) * 1e10
    maxB = _np.ones(nB, dtype=float) * -1e10
    ...
    minA = _np.ones(nA, dtype=float) * 1e10
    maxA = _np.ones(nA, dtype=float) * -1e10
    ...
    minD = _np.ones(nD, dtype=float) * 1e10
    maxD = _np.ones(nD, dtype=float) * -1e10
    ...
    while True:
        ...
        if N >= start and N <= stop:
            ...
            for i in range(nNB):
                minR[i] = min(minR[i], min(pairData[i]))
                maxR[i] = max(maxR[i], max(pairData[i]))
                Hist = _np.histogram(pairData[i], bins=xRDF)[0]
                nIntract = len(sysTop.NonBONDED_SetIdx[i])
                RDF[i] = RDF[i] + (volume / nIntract) * (Hist / jacob)
            for i in range(nB):
                minB[i] = min(minB[i], min(bondData[i]))
                maxB[i] = max(maxB[i], max(bondData[i]))
                Hist = _np.histogram(bondData[i], bins=xBDF)[0]
                BDF[i] = BDF[i] + Hist
            for i in range((nA)):
                minA[i] = min(minA[i], min(AngleData[i]))
                maxA[i] = max(maxA[i], max(AngleData[i]))
                Hist = _np.histogram(AngleData[i], bins=xADF)[0]
                ADF[i] = ADF[i] + Hist
            for i in range((nD)):
                minD[i] = min(minD[i], min(DihedralData[i]))
                maxD[i] = max(maxD[i], max(DihedralData[i]))
        ...
        if N == start:
            print('\nStart calculating the distribution functions')
    if CGtraj.time:
    ...
    sysTab.write()
    print('         {}'.format(outHL))
    with open(outHL, mode='w') as fid:
        for i in range(len(sysTop.NonBONDED_Set)):
            t1 = sysTop.NonBONDED_Set[i, 0]
            t2 = sysTop.NonBONDED_Set[i, 1]
            fid.write('NAME = {}\n'.format('Non-Bonded_' + t1 + '-' + t2))
            fid.write('TYPE = {} , {}\n'.format(t1, t2))
            fid.write('Minimum Distance = {0:.3f}\n'.format(minR[i]))
            fid.write('Maximum Distance = {0:.3f}\n'.format(maxR[i]))
            fid.write('Bin Width = {0:.3f}\n\n'.format(BinNB))
        if sysTop.BONDtypeSet is not None:
            for i in range(len(sysTop.BONDtypeSet)):
                if i == 0:
                    fid.write('###################\n###################\n\n')
                n1 = sysTop.BONDtypeSetName[i, 0]
                n2 = sysTop.BONDtypeSetName[i, 1]
                fid.write('NAME = {}\n'.format('Bond_' + n1 + '-' + n2))
                fid.write('TYPE = {}\n'.format(sysTop.BONDtypeSet[i]))
                fid.write('Minimum Distance = {0:.3f}\n'.format(minB[i]))
                fid.write('Maximum Distance = {0:.3f}\n'.format(maxB[i]))
                fid.write('Bin Width = {0:.3f}\n\n'.format(BinB))
        if sysTop.ANGLEtypeSet is not None:
            for i in range(len(sysTop.ANGLEtypeSet)):
                if i == 0:
                    fid.write('###################\n###################\n\n')
                n1 = sysTop.ANGLEtypeSetName[i, 0]
                n2 = sysTop.ANGLEtypeSetName[i, 1]
                n3 = sysTop.ANGLEtypeSetName[i, 2]
                fid.write('NAME = {}\n'.format('Angle_' + n1 + '-' + n2 + '-' + n3))
                fid.write('TYPE = {}\n'.format(sysTop.ANGLEtypeSet[i]))
                fid.write('Minimum Angle = {0:.3f}\n'.format(minA[i]))
                fid.write('Maximum Angle = {0:.3f}\n'.format(maxA[i]))
                fid.write('Bin Width = {0:.3f}\n\n'.format(BinA))
        if sysTop.DIHEDRALtypeSet is not None:
            for i in range(len(sysTop.DIHEDRALtypeSet)):
                if i == 0:
                    fid.write('###################\n###################\n\n')
                n1 = sysTop.DIHEDRALtypeSetName[i, 0]
                n2 = sysTop.DIHEDRALtypeSetName[i, 1]
                n3 = sysTop.DIHEDRALtypeSetName[i, 2]
                n4 = sysTop.DIHEDRALtypeSetName[i, 3]
                fid.write('NAME = {}\n'.format(
                    'Dihedral_' + n1 + '-' + n2 + '-' + n3 + '-' + n4))
                fid.write('TYPE = {}\n'.format(sysTop.DIHEDRALtypeSet[i]))
                fid.write('Minimum Angle = {0:.3f}\n'.format(minD[i]))
                fid.write('Maximum Angle = {0:.3f}\n'.format(maxD[i]))
                fid.write('Bin Width = {0:.3f}\n\n'.format(BinD))
...
• The figure saved for each table (*.png)'''
    ...
    isFORCE = _np.zeros(nInTabs, dtype=bool)
    for i in range(nInTabs):
        ...
        isFORCE[i] = inTabs[i].endswith('.force')
    ...
    for i in range(nNB):
        ...
        title = 'Non-Bonded: {0:s}-{1:s}'.format(NonBondType[i, 0], NonBondType[i, 1])
        ...
        if fType == 'HIST' or fType == 'DIST':
            ...
            _plt.ylabel('Force (kcal/mol/Angstrom)')#, fontsize=14)
        _plt.autoscale(True, axis=u'both', tight=True)
        _plt.title(title)
        _plt.legend(legend)
        if fType == 'HIST' or fType == 'DIST':
            _plt.savefig(Name + '.df.png', dpi=300)
        elif fType == 'POT':
            _plt.savefig(Name + '.pot.png', dpi=300)
        elif fType == 'DPOT':
            _plt.savefig(Name + '.dpot.png', dpi=300)
        elif fType == 'FORCE':
            _plt.savefig(Name + '.force.png', dpi=300)
        _plt.clf()
    ...
    for i in range(nB):
        ...
        for n in range(nInTabs):
            ...
            if nLeg < 11:
                ...
                _plt.plot(x, y, color=colorId[3 * n:3 * n + 3])
        title = 'Bond: {0:s}-{1:s}'.format(BondName[i, 0], BondName[i, 1])
        _plt.xlabel('r (Angstrom)')#, fontsize=14)
        if fType == 'HIST' or fType == 'DIST':
    ...
    for i in range(nA):
        ...
        for n in range(nInTabs):
            ...
            if nLeg < 11:
                ...
                _plt.plot(x, y, color=colorId[3 * n:3 * n + 3])
        title = 'Angle: {0:s}-{1:s}-{2:s}'.format(AngleName[i, 0], AngleName[i, 1], AngleName[i, 2])
        ...
        if fType == 'HIST' or fType == 'DIST':
            ...
            _plt.ylabel('Force (kcal/mol)')#, fontsize=14)
        _plt.autoscale(True, axis=u'both', tight=True)
        _plt.title(title)
        _plt.legend(legend)
        if fType == 'HIST' or fType == 'DIST':
    ...
    for i in range(nD):
        ...
        for n in range(nInTabs):
            ...
            if nLeg < 11:
                ...
                _plt.plot(x, y, color=colorId[3 * n:3 * n + 3])
        title = 'Dihedral: {0:s}-{1:s}-{2:s}-{3:s}'.format(DihedralName[i, 0], DihedralName[i, 1],
                                                         DihedralName[i, 2], DihedralName[i, 3])
...
• The PSF file ([output].psf)'''
    ...
    with open(output  + '.data.in', 'w') as dataFile:
        ...
        for i in range(nBeads):
            dataFile.write('{0:7d} {1:7d} {2:5d} {3:.3f} {4:e} {5:e} {6:e}\n'.format(i + 1, sysTop.MOLnum[i] + 1,
...
• Step, CPU, TotEng, KinEng, Temp, PotEng, E bond, E angle, E dihed, E impro, E vdwl, E coul, E long, Press, Volume'''
    ...
    while l < len(logL):
        line = logL[l]
        if ('Step' in line) and ('CPU' in line) and ('sec' in line) and (len(line.split()) == 9):
            ...
            if l < len(logL):
                ...
                break
            if ('TotEng' in line) and ('KinEng' in line) and ('Temp' in line) and (len(line.split()) == 9):
                ...
                if l < len(logL):
                    ...
                    break
                if ('PotEng' in line) and ('E_bond' in line) and ('E_angle' in line) and (len(line.split()) == 9):
                    ...
                    if l < len(logL):
                        ...
                        break
                    if ('E_dihed' in line) and ('E_impro' in line) and ('E_vdwl' in line) and (len(line.split()) == 9):
                        ...
                        if l < len(logL):
                            ...
                            break
                        if ('E_coul' in line) and ('E_long' in line) and ('Press' in line) and (len(line.split()) == 9):
                            ...
                            if l < len(logL):
                                ...
                                break
                            if ('Volume' in line) and (len(line.split()) == 3):
...
def ForceMatching_old(setFile):

    def kernel(x, x0, method, periodic=False):
        if periodic:
            x = _np.deg2rad(x)
            x0 = _np.deg2rad(x0)
            eps = _np.mean(_np.diff(x))
            dist = _np.abs(x - x0)
            arg = (_np.sin(dist / 2) / eps) ** 2
            return _np.exp(-1 * arg)
        eps = _np.mean(_np.diff(x))
        dist = x - x0
        arg = (dist / eps) ** 2
    ...
    #########################
    #########################
    def rbf_interp(x, weight, method, periodic=False):
        if periodic:
            x = _np.deg2rad(x)
        itpMat = _np.zeros([len(x), len(x)], dtype='float32')
        eps = _np.mean(_np.diff(x))
        for i in range(len(x)):
            itpMat[:, i] = _np.abs(x - x[i])
        if periodic:
            arg = (_np.sin(itpMat / 2) / eps) ** 2
        else:
            arg = (itpMat / eps) ** 2
        if method == 'GAUSSIAN':
            return _np.matmul(_np.exp(-1 * arg), weight)
        elif method == 'MULTIQD':
            return _np.matmul(_np.sqrt(1 + arg), weight)
        elif method == 'INVERSE':
            return _np.matmul(1 / (1 + arg), weight)
    #########################
    #########################
    def calcBondForce():
        for n in range(len(tList)):
            for i in range(len(sysTop.BONDtypeSetIdx)):
                indA = sysTop.BONDtypeSetIdx[i][:, 0]
                indB = sysTop.BONDtypeSetIdx[i][:, 1]
                xA, yA, zA = tList[n]['x'][indA], tList[n]['y'][indA], tList[n]['z'][indA]
                xB, yB, zB = tList[n]['x'][indB], tList[n]['y'][indB], tList[n]['z'][indB]
                dx = xA - xB
                dy = yA - yB
                dz = zA - zB
                dist = _np.sqrt(dx ** 2 + dy ** 2 + dz ** 2)
                ux = dx / dist
                uy = dy / dist
                uz = dz / dist
                if exclB:
                    X, Pot, Force = inPotForce_B[i]
                    dUdX = _np.interp(dist, X, Force)
                    fxA = dUdX * ux
                    fyA = dUdX * uy
                    fzA = dUdX * uz
                    fxB = -1 * fxA
                    ...
                    for j in range(len(dist)):
                        iA = indA[j]
                        iB = indB[j]
                        tfList[n]['fx'][iA] -= fxA[j]
                        tfList[n]['fy'][iA] -= fyA[j]
                        tfList[n]['fz'][iA] -= fzA[j]
                        tfList[n]['fx'][iB] -= fxB[j]
                        tfList[n]['fy'][iB] -= fyB[j]
                        tfList[n]['fz'][iB] -= fzB[j]
                else:
                    x = B_x[i]
                    c0, c1 = B_wgt_ind[i]
                    nId = n * nBeads * 3
                    for j in range(len(dist)):
                        dUdX = kernel(x, dist[j], function)
                        fxA = dUdX * ux[j]
                        fyA = dUdX * uy[j]
                        fzA = dUdX * uz[j]
                        fxB = -1 * fxA
                        ...
                        iA = indA[j]
                        iB = indB[j]
                        distMat[nId + iA * 3, c0:c1] += fxA
                        distMat[nId + iA * 3 + 1, c0:c1] += fyA
                        distMat[nId + iA * 3 + 2, c0:c1] += fzA
                        distMat[nId + iB * 3, c0:c1] += fxB
                        distMat[nId + iB * 3 + 1, c0:c1] += fyB
                        distMat[nId + iB * 3 + 2, c0:c1] += fzB
    ###########################################################################
    ###########################################################################
    def calcAngleForce():
        for n in range(len(tList)):
            for i in range(len(sysTop.ANGLEtypeSetIdx)):
                indA = sysTop.ANGLEtypeSetIdx[i][:, 0]
                indB = sysTop.ANGLEtypeSetIdx[i][:, 1]
                indC = sysTop.ANGLEtypeSetIdx[i][:, 2]
                xA, yA, zA = tList[n]['x'][indA], tList[n]['y'][indA], tList[n]['z'][indA]
                xB, yB, zB = tList[n]['x'][indB], tList[n]['y'][indB], tList[n]['z'][indB]
                xC, yC, zC = tList[n]['x'][indC], tList[n]['y'][indC], tList[n]['z'][indC]
                BA = _np.zeros([len(xB), 3], dtype='float32')
                BA[:, 0] = xA - xB
                BA[:, 1] = yA - yB
                BA[:, 2] = zA - zB
                BC = _np.zeros([len(xB), 3], dtype='float32')
                BC[:, 0] = xC - xB
                BC[:, 1] = yC - yB
                BC[:, 2] = zC - zB
                ...
                ctheta[ctheta > 1.0] = 1.0
                ctheta[ctheta < -1.0] = -1.0
                theta = _np.rad2deg(_np.arccos(ctheta))

                P1 = _np.cross(BA, _np.cross(BA, BC))
                nrm = _np.linalg.norm(P1, axis=1)
                P1 = P1 / (nrm * ba).repeat(3).reshape([len(nrm), 3])

                P3 = _np.cross(-1 * BC, _np.cross(BA, BC))
                nrm = _np.linalg.norm(P3, axis=1)
                P3 = P3 / (nrm * bc).repeat(3).reshape([len(nrm), 3])
                if exclA:
                    X, Pot, Force = inPotForce_A[i]
                    dUdX = _np.interp(theta, X, Force)
                    fxA = dUdX * P1[:, 0]
                    fyA = dUdX * P1[:, 1]
                    fzA = dUdX * P1[:, 2]
                    fxC = dUdX * P3[:, 0]
                    ...
                    for j in range(len(theta)):
                        ...
                        tfList[n]['fy'][iB] -= fyB[j]
                        tfList[n]['fz'][iB] -= fzB[j]
                        tfList[n]['fx'][iC] -= fxC[j]
                        tfList[n]['fy'][iC] -= fyC[j]
                        tfList[n]['fz'][iC] -= fzC[j]
                else:
                    x = A_x[i]
                    c0, c1 = A_wgt_ind[i]
                    nId = n * nBeads * 3
                    for j in range(len(theta)):
                        dUdX = kernel(x, theta[j], function)
                        fxA = dUdX * P1[j, 0]
                        fyA = dUdX * P1[j, 1]
                        fzA = dUdX * P1[j, 2]
                        fxC = dUdX * P3[j, 0]
                        ...
                        distMat[nId + iB * 3 + 1, c0:c1] += fyB
                        distMat[nId + iB * 3 + 2, c0:c1] += fzB
                        distMat[nId + iC * 3, c0:c1] += fxC
                        distMat[nId + iC * 3 + 1, c0:c1] += fyC
                        distMat[nId + iC * 3 + 2, c0:c1] += fzC
    ###########################################################################
    ###########################################################################
    def calcDihedralForce():
        for n in range(len(tList)):
            for i in range(len(sysTop.DIHEDRALtypeSetIdx)):
                indA = sysTop.DIHEDRALtypeSetIdx[i][:, 0]
                indB = sysTop.DIHEDRALtypeSetIdx[i][:, 1]
                indC = sysTop.DIHEDRALtypeSetIdx[i][:, 2]
                indD = sysTop.DIHEDRALtypeSetIdx[i][:, 3]
                xA, yA, zA = tList[n]['x'][indA], tList[n]['y'][indA], tList[n]['z'][indA]
                xB, yB, zB = tList[n]['x'][indB], tList[n]['y'][indB], tList[n]['z'][indB]
                xC, yC, zC = tList[n]['x'][indC], tList[n]['y'][indC], tList[n]['z'][indC]
                xD, yD, zD = tList[n]['x'][indD], tList[n]['y'][indD], tList[n]['z'][indD]
                BA = _np.zeros([len(xB), 3], dtype='float32')
                BA[:, 0] = xA - xB
                BA[:, 1] = yA - yB
                BA[:, 2] = zA - zB
                ...
                BC[:, 2] = zC - zB
                CB = -1 * BC
                CD = _np.zeros([len(xB), 3], dtype='float32')
                CD[:, 0] = xD - xC
                CD[:, 1] = yD - yC
                CD[:, 2] = zD - zC
                ba = _np.linalg.norm(BA, axis=1)
                ...
                ctheta1 = _np.sum(BA * BC, axis=1) / (ba * bc)
                ctheta1[ctheta1 > 1.0] = 1.0
                ctheta1[ctheta1 < -1.0] = -1.0
                theta1 = _np.arccos(ctheta1)

                ctheta2 = _np.sum(CB * CD, axis=1) / (bc * cd)
                ctheta2[ctheta2 > 1.0] = 1.0
                ctheta2[ctheta2 < -1.0] = -1.0
                theta2 = _np.arccos(ctheta2)

                P1 = _np.cross(BA, BC)
                nrm = _np.linalg.norm(P1, axis=1)
                P1 = P1 / nrm.repeat(3).reshape([len(nrm), 3])
                P1 = P1 / (ba * _np.sin(theta1)).repeat(3).reshape([len(ba), 3])

                P4 = _np.cross(CD, CB)
                nrm = _np.linalg.norm(P4, axis=1)
                P4 = P4 / nrm.repeat(3).reshape([len(nrm), 3])
                P4 = P4 / (cd * _np.sin(theta2)).repeat(3).reshape([len(cd), 3])

                OC = _np.zeros([len(xC), 3], dtype='float32')
                OC[:, 0] = xC - 0.5 * (xC + xD)
                OC[:, 1] = yC - 0.5 * (yC + yD)
                OC[:, 2] = zC - 0.5 * (zC + zD)
                oc = _np.linalg.norm(OC, axis=1)
                oc2 = (oc ** 2).repeat(3).reshape([len(oc), 3])
                if exclD:
                    X, Pot, Force = inPotForce_D[i]
                    dUdX = _np.interp(phi, X, Force)
                    fxA = dUdX * P1[:, 0]
                    fyA = dUdX * P1[:, 1]
                    fzA = dUdX * P1[:, 2]
                    fxD = dUdX * P4[:, 0]
                    fyD = dUdX * P4[:, 1]
                    fzD = dUdX * P4[:, 2]
                    fA = _np.zeros([len(fxA), 3], dtype='float32')
                    fA[:, 0] = fxA
                    fA[:, 1] = fyA
                    fA[:, 2] = fzA
                    fD = _np.zeros([len(fxD), 3], dtype='float32')
                    fD[:, 0] = fxD
                    fD[:, 1] = fyD
                    fD[:, 2] = fzD

                    tC = -1 * (_np.cross(OC, fD) + 0.5 * (_np.cross(CD, fD) + _np.cross(BA, fA)))
                    fC = _np.cross(tC, OC) / oc2
                    fxC = fC[:, 0]
                    fyC = fC[:, 1]
                    fzC = fC[:, 2]
                    ...
                    for j in range(len(phi)):
                        ...
                        tfList[n]['fy'][iC] -= fyC[j]
                        tfList[n]['fz'][iC] -= fzC[j]
                        tfList[n]['fx'][iD] -= fxD[j]
                        tfList[n]['fy'][iD] -= fyD[j]
                        tfList[n]['fz'][iD] -= fzD[j]
                else:
                    x = D_x[i]
                    c0, c1 = D_wgt_ind[i]
                    nId = n * nBeads * 3
                    for j in range(len(phi)):
                        dUdX = kernel(x, phi[j], function, periodic=False)
                        fxA = dUdX * P1[j, 0]
                        fyA = dUdX * P1[j, 1]
                        fzA = dUdX * P1[j, 2]
                        fxD = dUdX * P4[j, 0]
                        ...
                        yBA = BA[j, 1]
                        zBA = BA[j, 2]
                        xOCxfD = yOC * fzD - zOC * fyD
                        yOCxfD = zOC * fxD - xOC * fzD
                        zOCxfD = xOC * fyD - yOC * fxD
                        xCDxfD = yCD * fzD - zCD * fyD
                        yCDxfD = zCD * fxD - xCD * fzD
                        zCDxfD = xCD * fyD - yCD * fxD
                        xBAxfA = yBA * fzA - zBA * fyA
                        yBAxfA = zBA * fxA - xBA * fzA
                        zBAxfA = xBA * fyA - yBA * fxA

                        x_tC = -1 * (xOCxfD + 0.5 * (xCDxfD + xBAxfA))
                        y_tC = -1 * (yOCxfD + 0.5 * (yCDxfD + yBAxfA))
                        z_tC = -1 * (zOCxfD + 0.5 * (zCDxfD + zBAxfA))
                        fxC = (y_tC * zOC - z_tC * yOC) / oc2[j, 0]
                        fyC = (z_tC * xOC - x_tC * zOC) / oc2[j, 0]
                        fzC = (x_tC * yOC - y_tC * xOC) / oc2[j, 0]
                        fxB = -fxA - fxC - fxD
                        fyB = -fyA - fyC - fyD
                        fzB = -fzA - fzC - fzD
                        ...
                        distMat[nId + iC * 3 + 1, c0:c1] += fyC
                        distMat[nId + iC * 3 + 2, c0:c1] += fzC
                        distMat[nId + iD * 3, c0:c1] += fxD
                        distMat[nId + iD * 3 + 1, c0:c1] += fyD
                        distMat[nId + iD * 3 + 2, c0:c1] += fzD
    ###########################################################################
    ###########################################################################
    def calcNonBondedForce():
        for n in range(len(tList)):
            boxMat = tList[n]['boxMat']
            if len(boxMat) == 15:
                lx = boxMat['xhi'] - boxMat['xlo']
                ly = boxMat['yhi'] - boxMat['ylo']
                lz = boxMat['zhi'] - boxMat['zlo']
                ...
                lx, ly, lz, xy, xz, yz = boxMat[0], boxMat[1], boxMat[2], \
                                         boxMat[3], boxMat[4], boxMat[5]
            for i in range(len(sysTop.NonBONDED_SetIdx)):
                indA = sysTop.NonBONDED_SetIdx[i][:, 0]
                indB = sysTop.NonBONDED_SetIdx[i][:, 1]
                xA, yA, zA = tList[n]['xs'][indA], tList[n]['ys'][indA], tList[n]['zs'][indA]
                xB, yB, zB = tList[n]['xs'][indB], tList[n]['ys'][indB], tList[n]['zs'][indB]
                dx = xA - xB - _np.rint(xA - xB)
                dy = yA - yB - _np.rint(yA - yB)
                dz = zA - zB - _np.rint(zA - zB)
                dX = dx * lx + dy * xy + dz * xz
                dY = dy * ly + dz * yz
                dZ = dz * lz
                R = _np.sqrt(dX ** 2 + dY ** 2 + dZ ** 2)
                ux = dX / R
                uy = dY / R
                uz = dZ / R

                x = _np.zeros([1, len(NB_x[i])], dtype='float32')
                x[0] = NB_x[i]
                x_mat = _np.repeat(x, len(R), axis=0)
                R_mat = R.repeat(len(x[0])).reshape([len(R), len(x[0])])
                eps = _np.mean(_np.diff(x[0]))
                arg = ((x_mat - R_mat) / eps) ** 2
                if function == 'GAUSSIAN':
                    dUdX_mat = _np.exp(-1 * arg)
                elif function == 'MULTI':
                    dUdX_mat = _np.sqrt(1 + arg)
                elif function == 'INVERSE':
                    dUdX_mat = 1 / (1 + arg)
                fxA = dUdX_mat * ux.repeat(len(x[0])).reshape([len(ux), len(x[0])])
                fyA = dUdX_mat * uy.repeat(len(x[0])).reshape([len(uy), len(x[0])])
                fzA = dUdX_mat * uz.repeat(len(x[0])).reshape([len(uz), len(x[0])])
                fxB = -1 * fxA
                fyB = -1 * fyA
                fzB = -1 * fzA
                c0, c1 = NB_wgt_ind[i]
                nId = n * nBeads * 3
                indA_unq = _np.unique(indA)
                indB_unq = _np.unique(indB)
                for j in range(len(indA_unq)):
                    iA = indA_unq[j]
                    ids = _np.where(indA == iA)[0]
                    fx = _np.sum(fxA[ids], axis=0)
                    fy = _np.sum(fyA[ids], axis=0)
                    fz = _np.sum(fzA[ids], axis=0)
                    distMat[nId + iA * 3, c0:c1] += fx
                    distMat[nId + iA * 3 + 1, c0:c1] += fy
                    distMat[nId + iA * 3 + 2, c0:c1] += fz
                for j in range(len(indB_unq)):
                    iB = indB_unq[j]
                    ids = _np.where(indB == iB)[0]
                    fx = _np.sum(fxB[ids], axis=0)
                    fy = _np.sum(fyB[ids], axis=0)
                    fz = _np.sum(fzB[ids], axis=0)
                    distMat[nId + iB * 3, c0:c1] += fx
                    distMat[nId + iB * 3 + 1, c0:c1] += fy
                    distMat[nId + iB * 3 + 2, c0:c1] += fz
    ###########################################################################
    ###########################################################################
    def genForceVec():
        nComp = nBeads * 3
        forceVec = _np.zeros(nComp * len(tfList), dtype='float32')
        for n in range(len(tfList)):
            fArr = _np.zeros([nBeads, 3], dtype='float32')
            fArr[:, 0] = tfList[n]['fx']
            fArr[:, 1] = tfList[n]['fy']
            fArr[:, 2] = tfList[n]['fz']
            forceVec[n*nComp: (n+1)*nComp] = fArr.reshape(nComp)
        return forceVec
    #########################
    #########################
    def wgt2force():
        forceBlock = _np.zeros(n_wgt, dtype='float32')
        for i in range(nNB):
            x = NB_x[i]
            c0, c1 = NB_wgt_ind[i]
            weight = wgt[c0:c1]
            forceBlock[c0:c1] = rbf_interp(x, weight, function)
        if not exclB:
            for i in range(nB):
                x = B_x[i]
                c0, c1 = B_wgt_ind[i]
        ...
        if not exclD:
            for i in range(nD):
                ...
                c0, c1 = D_wgt_ind[i]
                weight = wgt[c0:c1]
                forceBlock[c0:c1] = rbf_interp(x, weight, function, periodic=True)
        return forceBlock
    #########################
    #########################
    def writeLog(fName, init=False):
        if init:
            with open(fName, 'w') as fid:
                for i in range(nNB):
                    t1 = sysTop.NonBONDED_Set[i, 0]
                    t2 = sysTop.NonBONDED_Set[i, 1]
                    fid.write('Non-Bonded: {}-{}\n'.format(t1, t2))
                    fid.write('  min_index = {}\n'.format(NB_wgt_ind[i, 0]))
                    fid.write('  max_index = {}\n'.format(NB_wgt_ind[i, 1]))
                    fid.write('  R = ')
                    for j in range(len(NB_x[i])):
                        fid.write('{:e} '.format(NB_x[i][j]))
                    fid.write('\n\n')
                if not exclB:
                    for i in range(nB):
                        t = sysTop.BONDtypeSet[i] + 1
                        n1 = sysTop.BONDtypeSetName[i, 0]
                        n2 = sysTop.BONDtypeSetName[i, 1]
                        fid.write('Bond-{}: {}-{}\n'.format(t, n1, n2))
                        fid.write('  min_index = {}\n'.format(B_wgt_ind[i, 0]))
                        fid.write('  max_index = {}\n'.format(B_wgt_ind[i, 1]))
                        fid.write('  R = ')
                        for j in range(len(B_x[i])):
                            fid.write('{:e} '.format(B_x[i][j]))
                        fid.write('\n\n')
                if not exclA:
                    for i in range(nA):
                        t = sysTop.ANGLEtypeSet[i] + 1
                        n1 = sysTop.ANGLEtypeSetName[i, 0]
                        n2 = sysTop.ANGLEtypeSetName[i, 1]
                        n3 = sysTop.ANGLEtypeSetName[i, 2]
                        fid.write('Angle-{}: {}-{}-{}\n'.format(t, n1, n2, n3))
                        fid.write('  min_index = {}\n'.format(A_wgt_ind[i, 0]))
                        fid.write('  max_index = {}\n'.format(A_wgt_ind[i, 1]))
                        fid.write('  theta = ')
                        for j in range(len(A_x[i])):
                            fid.write('{:e} '.format(A_x[i][j]))
                        fid.write('\n\n')
                if not exclD:
                    for i in range(nD):
                        t = sysTop.DIHEDRALtypeSet[i] + 1
                        n1 = sysTop.DIHEDRALtypeSetName[i, 0]
                        n2 = sysTop.DIHEDRALtypeSetName[i, 1]
                        n3 = sysTop.DIHEDRALtypeSetName[i, 2]
                        n4 = sysTop.DIHEDRALtypeSetName[i, 3]
                        fid.write('Dihedral-{}: {}-{}-{}-{}\n'.format(t, n1, n2, n3, n4))
                        fid.write('  min_index = {}\n'.format(D_wgt_ind[i, 0]))
                        fid.write('  max_index = {}\n'.format(D_wgt_ind[i, 1]))
                        fid.write('  phi = ')
                        for j in range(len(D_x[i])):
                            fid.write('{:e} '.format(D_x[i][j]))
                        fid.write('\n\n')
                fid.write('##############################\n##############################\n\n')
                fid.write('minimum acceptable block size = {}\n'.format(bz_min))
                fid.write('current block size = {}\n\n'.format(bz))
        else:
            with open(fName, 'a') as fid:
                for i in range(len(forceBlock)):
                    fid.write('{:e} '.format(forceBlock[i]))
                fid.write('\n')
    #########################
    #########################
    T1 = _time.time()
    ...
    binAD = 0.05
    binB = 0.01
    exclB = exclA = exclD = False
    for line in optF:
        lsp = line.split('#')[0]
        lsp0 = lsp.split('=')[0].strip()
        if 'TOP' == lsp0.upper():
            top = lsp.split('=')[1].strip()
        elif 'HLOG' == lsp0.upper():
            hLog = lsp.split('=')[1].strip()
        elif 'REFHIST' == lsp0.upper():
            refHist = lsp.split('=')[1].strip()
        elif 'CGTRAJ' == lsp0.upper():
            CGtraj = lsp.split('=')[1].strip()
        elif 'AAFORCE' == lsp0.upper():
            AAforce = lsp.split('=')[1].strip()
        elif 'MAPPING' == lsp0.upper():
            mapFile = lsp.split('=')[1].strip()
        elif 'OUTPUT' == lsp0.upper():
            output = lsp.split('=')[1].strip()
        elif 'CUTOFF' == lsp0.upper():
            cutoff = float(lsp.split('=')[1].strip())
        elif 'START' == lsp0.upper():
            startP = int(lsp.split('=')[1].strip())
        elif 'NSNAP' == lsp0.upper():
            nsnap2read = int(lsp.split('=')[1].strip())
        elif 'BLOCK' == lsp0.upper():
            block = int(lsp.split('=')[1].strip())
        elif 'FUNCTION' == lsp0.upper():
            ss = lsp.split('=')[1].strip().upper()
            if ss == 'MULTIQUADRATIC':
                function = 'MULTIQD'
            elif ss == 'INVERSE':
                function = 'INVERSE'
            elif ss == 'GAUSSIAN':
                ...
                raise IOError('Supported functions: [ multiquadric, inverse, gaussian ]; the default is "multiquadric"')
        elif 'EXCLB' == lsp0.upper():
            if lsp.split('=')[1].strip().upper() == 'YES':
                exclB = True
            elif lsp.split('=')[1].strip().upper() == 'NO':
                exclB = False
        elif 'EXCLA' == lsp0.upper():
            if lsp.split('=')[1].strip().upper() == 'YES':
                exclA = True
            elif lsp.split('=')[1].strip().upper() == 'NO':
                exclA = False
        elif 'EXCLD' == lsp0.upper():
            if lsp.split('=')[1].strip().upper() == 'YES':
                exclD = True
            elif lsp.split('=')[1].strip().upper() == 'NO':
                exclD = False
        elif 'INPOT' == lsp0.upper():
            inPot = lsp.split('=')[1].strip()
        elif 'MAXB' == lsp0.upper():
            maxB = float(lsp.split('=')[1].strip())
        elif 'MAXA' == lsp0.upper():
            maxA = float(lsp.split('=')[1].strip())
        elif 'BINAD' == lsp0.upper():
            binAD = float(lsp.split('=')[1].strip())
        elif 'BINB' == lsp0.upper():
            binB = float(lsp.split('=')[1].strip())
    if exclB or exclA or exclD:
        if inPot is None:
            raise IOError('input potentials not defined')
    ############
    if startP < 1:
        raise ValueError('"start > 0"')
    if nsnap2read > 1:
        ...
        raise ValueError('"nsnap > 0" or' "nsnap = -1")
    ###############
    sysMap = _cMAP(mapFile)
    AtomCumSum = _np.append([0], _np.cumsum(sysMap.nATOM * sysMap.nMOL))
    newBEADmap = _np.zeros(len(sysMap.MOL), dtype=object)
    for mol in range(len(sysMap.MOL)):
        newBEADmap[mol] = sysMap.BEADmap[mol] + AtomCumSum[mol]
    nBeads = int(_np.sum(sysMap.nMOL * sysMap.nBEAD))
    nAtoms = int(_np.sum(sysMap.nMOL * sysMap.nATOM))
    BeadsMap = _np.zeros(nBeads, dtype=object)
    m = 0
    for mol in range(len(sysMap.MOL)):
        for n in range(sysMap.nMOL[mol]):
            BeadsMap[m: m + sysMap.nBEAD[mol]] = newBEADmap[mol] + n * sysMap.nATOM[mol]
            m += sysMap.nBEAD[mol]
    ###################
    sysTop = _cTOP(top)
    sysTop.SetBondedIndex()
    ...
    #     for i in range(len(sysTop.NonBONDED_Set)):
    #         fid.write('NonBonded: {}_{}\n'.format(sysTop.NonBONDED_Set[i, 0], sysTop.NonBONDED_Set[i, 1]))
    #         fid.write('   len_R          : {}\n'.format(len(sysTop.NonBONDED_SetIdx[i])))
    #         u_a = len(_np.unique(sysTop.NonBONDED_SetIdx[i][:, 0]))
    #         u_b = len(_np.unique(sysTop.NonBONDED_SetIdx[i][:, 1]))
    #         fid.write('   len_unique_indA: {}\n'.format(u_a))
    #         fid.write('   len_unique_indB: {}\n'.format(u_b))
    #         fid.write('   len_unique_A_B : {}\n'.format(u_a + u_b))
    #     for i in range(len(sysTop.BONDtypeSet)):
    #         fid.write('Bond: {}\n'.format(sysTop.BONDtypeSet[i]))
    #         fid.write('   len_R          : {}\n'.format(len(sysTop.BONDtypeSetIdx[i])))
    #         u_a = len(_np.unique(sysTop.BONDtypeSetIdx[i][:, 0]))
    #         u_b = len(_np.unique(sysTop.BONDtypeSetIdx[i][:, 1]))
    #         fid.write('   len_unique_indA: {}\n'.format(u_a))
    #         fid.write('   len_unique_indB: {}\n'.format(u_b))
    #         fid.write('   len_unique_A_B : {}\n'.format(u_a + u_b))
    #     for i in range(len(sysTop.ANGLEtypeSet)):
    #         fid.write('Angle: {}\n'.format(sysTop.ANGLEtypeSet[i]))
    #         fid.write('   len_R          : {}\n'.format(len(sysTop.ANGLEtypeSetIdx[i])))
    #         u_a = len(_np.unique(sysTop.ANGLEtypeSetIdx[i][:, 0]))
    #         u_b = len(_np.unique(sysTop.ANGLEtypeSetIdx[i][:, 1]))
    #         u_c = len(_np.unique(sysTop.ANGLEtypeSetIdx[i][:, 2]))
    #         fid.write('   len_unique_indA: {}\n'.format(u_a))
    #         fid.write('   len_unique_indB: {}\n'.format(u_b))
    #         fid.write('   len_unique_indC: {}\n'.format(u_c))
    #         fid.write('   len_unique_ABC : {}\n'.format(u_a + u_b + u_c))
    #     for i in range(len(sysTop.DIHEDRALtypeSet)):
    #         fid.write('Dihedral: {}\n'.format(sysTop.DIHEDRALtypeSet[i]))
    #         fid.write('   len_R          : {}\n'.format(len(sysTop.DIHEDRALtypeSetIdx[i])))
    #         u_a = len(_np.unique(sysTop.DIHEDRALtypeSetIdx[i][:, 0]))
    #         u_b = len(_np.unique(sysTop.DIHEDRALtypeSetIdx[i][:, 1]))
    #         u_c = len(_np.unique(sysTop.DIHEDRALtypeSetIdx[i][:, 2]))
    #         u_d = len(_np.unique(sysTop.DIHEDRALtypeSetIdx[i][:, 3]))
    #         fid.write('   len_unique_indA: {}\n'.format(u_a))
    #         fid.write('   len_unique_indB: {}\n'.format(u_b))
    #         fid.write('   len_unique_indC: {}\n'.format(u_c))
    #         fid.write('   len_unique_indD: {}\n'.format(u_d))
    #         fid.write('   len_unique_ABCD: {}\n'.format(u_a + u_b + u_c + u_d))

    ######
    ######
    #########################
    sys_hLog = _cTAB(hLog, 'r')
    if refHist is not None:
        sys_refHist = _cTAB(refHist, 'r')
    if exclB or exclA or exclD:
        sys_pot = _cTAB(inPot, 'r')
    n_wgt = 0
    nNB = len(sysTop.NonBONDED_Set)
    NB_x = _np.zeros(nNB, dtype=object)
    NB_bw = _np.zeros(nNB, dtype=float)
    NB_wgt_ind = _np.zeros([nNB, 2], dtype=int)
    for i in range(nNB):
        if refHist is not None:
            bw = sys_refHist.NonBondBinW[i]
        else:
            bw = sys_hLog.NonBondBinW[i]
        minX = sys_hLog.NonBondMinMax[i, 0]
        maxX = sys_hLog.NonBondMinMax[i, 1]
        npoint = _np.int(_np.round(cutoff / bw)) + 1
        x = _np.linspace(0, cutoff, npoint, dtype=float)
        if minX <= x[0]:
            pass
        else:
            x = _np.append(x[x < minX][-1], x[x >= minX])
        if x[-1] > maxX:
            x = _np.append(x[x <= maxX], x[x > maxX][0])
        else:
            x = x[x <= maxX]
        NB_wgt_ind[i] = [n_wgt, n_wgt + len(x)]
        n_wgt += len(x)
        NB_x[i] = x
        NB_bw[i] = bw
    ...
    nB = len(sysTop.BONDtypeSet)
    if exclB:
        inPotForce_B = _np.zeros([nB, 3], dtype=object)
        for i in range(nB):
            X, Pot, Force = sys_pot.extrpPot_Bond(i, binB, maxB, cutoff)
            inPotForce_B[i] = [X, Pot, Force]
    else:
        B_x = _np.zeros(nB, dtype=object)
        B_bw = _np.zeros(nB, dtype=float)
        B_wgt_ind = _np.zeros([nB, 2], dtype=int)
        for i in range(nB):
            if refHist is not None:
                bw = sys_refHist.BondBinW[i]
            else:
                bw = sys_hLog.BondBinW[i]
            minX = sys_hLog.BondMinMax[i, 0]
            maxX = sys_hLog.BondMinMax[i, 1]
            npoint = _np.int(_np.round(40 / bw)) + 1
            x = _np.linspace(0, 40, npoint, dtype=float)
            if minX <= x[0]:
                pass
            else:
                x = _np.append(x[x < minX][-1], x[x >= minX])
    ...
    nA = len(sysTop.ANGLEtypeSet)
    if exclA:
        inPotForce_A = _np.zeros([nA, 3], dtype=object)
        for i in range(nA):
            X, Pot, Force = sys_pot.extrpPot_Angle(i, binAD, maxA)
            inPotForce_A[i] = [X, Pot, Force]
    else:
        A_x = _np.zeros(nA, dtype=object)
        A_bw = _np.zeros(nA, dtype=float)
        A_wgt_ind = _np.zeros([nA, 2], dtype=int)
        for i in range(nA):
            if refHist is not None:
                bw = sys_refHist.AngleBinW[i]
            else:
                bw = sys_hLog.AngleBinW[i]
            minX = sys_hLog.AngleMinMax[i, 0]
            maxX = sys_hLog.AngleMinMax[i, 1]
            npoint = _np.int(_np.round(180 / bw)) + 1
            x = _np.linspace(0, 180, npoint, dtype=float)
            if minX <= x[0]:
                pass
            else:
                x = _np.append(x[x < minX][-1], x[x >= minX])
    ...
    nD = len(sysTop.DIHEDRALtypeSet)
    if exclD:
        inPotForce_D = _np.zeros([nD, 3], dtype=object)
        for i in range(nD):
            X, Pot, Force = sys_pot.extrpPot_Dihedral(i, binAD)
            inPotForce_D[i] = [X, Pot, Force]
    else:
        D_x = _np.zeros(nD, dtype=object)
        D_bw = _np.zeros(nD, dtype=float)
        D_wgt_ind = _np.zeros([nD, 2], dtype=int)
        for i in range(nD):
            if refHist is not None:
                bw = sys_refHist.DihedralBinW[i]
            else:
                bw = sys_hLog.DihedralBinW[i]
            npoint = _np.int(_np.round(360 / bw)) + 1
            x = _np.linspace(-180, 180, npoint, dtype=float)
            D_wgt_ind[i] = [n_wgt, n_wgt + len(x)]
            n_wgt += len(x)
            D_x[i] = x
            D_bw[i] = bw

    forceMat = _np.zeros([0, n_wgt], dtype='float32')
    bz_min = int(_np.ceil(n_wgt / (nBeads * 3)))
    if block < bz_min:
        bz = max(bz_min, 10)
    else:
        bz = block
    writeLog(output + '.log', init=True)
    print('log file = {}.log'.format(output))
    print('minimum acceptable block size = {}\ncurrent block size = {}\n'.format(bz_min, bz))
    Traj = _cTRAJ(CGtraj, 'r')
    TrajForce = _cTRAJ(AAforce, 'r')
    NP = -1
    while NP + 1 < startP:
        NP += 1
        Traj.read(i=NP)
        if Traj.eof:
            Traj.close()
            raise IOError('startP is greater than number of snapshots.')

    NF = 0
    TrajForce.read(i=NF)
    checkTime = checkStep = False
    if TrajForce.time is not None and Traj.time is not None:
        checkTime = True
    elif TrajForce.timeStep is not None and Traj.timeStep is not None:
        checkStep = True
    else:
        raise IOError('data of time step was not found in the trajectories.')
    if checkTime:
        if Traj.time > TrajForce.time:
            while Traj.time != TrajForce.time:
                NF += 1
                TrajForce.read(i=NF)
                if TrajForce.eof:
                    TrajForce.close()
                    Traj.close()
                    mess = 'time [position] = {}\n'.format(Traj.time)
                    mess += 'time [force] = {}\n'.format(TrajForce.time)
                    mess += 'position and force trajectories are not synchronous.'
                    raise IOError(mess)
        elif Traj.time < TrajForce.time:
            while Traj.time != TrajForce.time:
                NP += 1
                Traj.read(i=NP)
                if Traj.eof:
        ...
        if Traj.timeStep > TrajForce.timeStep:
            while Traj.timeStep != TrajForce.timeStep:
                NF += 1
                TrajForce.read(i=NF)
                if TrajForce.eof:
                    TrajForce.close()
                    Traj.close()
                    mess = 'time step [position] = {}\n'.format(Traj.timeStep)
                    mess += 'time step [force] = {}\n'.format(TrajForce.timeStep)
                    mess += 'position and force trajectories are not synchronous.'
                    raise IOError(mess)
        elif Traj.timeStep < TrajForce.timeStep:
            while Traj.timeStep != TrajForce.timeStep:
                NP += 1
                Traj.read(i=NP)
                if Traj.eof:
    ...
    startP = NP
    startF = NF
    print('trajectory of positions started from {}-th snapshot'.format(startP + 1))
    print('trajectory of forces started from {}-th snapshot'.format(startF + 1))
    if checkTime:
        print('starting time :\n\tposition = {}\n\tforce    = {}'.format(Traj.time, TrajForce.time))
    elif checkStep:
        print('starting step :\n\tposition = {}\n\tforce    = {}'.format(Traj.timeStep, TrajForce.timeStep))
    ...
    while parseTrajs:
        ...
        flag = 0
        while flag < bz:
            if NP <= startP + nsnap2read and NF <= startF + nsnap2read:
                Traj.scaleSnap()
                if Traj.xu is not None:
                    x, y, z = Traj.xu, Traj.yu, Traj.zu
                elif Traj.x is not None:
                    x, y, z = Traj.x, Traj.y, Traj.z
                else:
                    raise IOError('Error in parsing the trajectory')
                dict1 = {}
                dict1['x'] = x
                dict1['y'] = y
                dict1['z'] = z
                ...
                dict1['ys'] = Traj.ys
                dict1['zs'] = Traj.zs
                dict1['boxMat'] = Traj.boxMat
                tList.append(dict1)
                fx = TrajForce.fx
                fy = TrajForce.fy
                fz = TrajForce.fz
                fxCG = _np.zeros(nBeads, dtype='float32')
                fyCG = _np.zeros(nBeads, dtype='float32')
                fzCG = _np.zeros(nBeads, dtype='float32')
                fv = _np.zeros(nBeads * 3, dtype='float32')
                for i in _np.arange(nBeads):
                    fxCG[i] = _np.sum(fx[BeadsMap[i]])
                    fyCG[i] = _np.sum(fy[BeadsMap[i]])
                    fzCG[i] = _np.sum(fz[BeadsMap[i]])
                dict2 = {}
                dict2['fx'] = fxCG
                dict2['fy'] = fyCG
                dict2['fz'] = fzCG
            ...
            try:
                ...
                mess += 'time [positions] = {}\n'.format(Traj.time)
                mess += 'time [forces] = {}\n'.format(TrajForce.time)
                mess += 'timeStep [positions] = {}\n'.format(Traj.timeStep)
                mess += 'timeStep [forces] = {}\n'.format(TrajForce.timeStep)
                mess += 'The trajectory of forces and positions are not synchronized.\n'
                raise IOError(mess)

        if parseTrajs and len(tList) == bz:
            nR = len(tList) * nBeads * 3
            distMat = _np.zeros([nR, n_wgt], dtype='float32')
            ################## bond
            calcBondForce()
            calcAngleForce()
            calcDihedralForce()
            calcNonBondedForce()
            forceVec = genForceVec()
            wgt = _np.linalg.pinv(distMat).dot(forceVec)
            forceBlock = wgt2force()
            forceMat = _np.append(forceMat, [forceBlock], axis=0)
            writeLog(output + '.log', init=False)
            T2 = _time.time()
            Sec = round((T2 - T1))
            Min, Sec = divmod(Sec, 60)
            Hour, Min = divmod(Min, 60)
            Hour, Min, Sec = int(Hour), int(Min), int(Sec)
            print('\rsnapshot = {0:^5d}  ---  elapsed time = {1:d}:{2:02d}:{3:02d}'.format(NP, Hour, Min, Sec), end='')
    del distMat, tList, tfList, forceVec, forceBlock
    forceAve = _np.mean(forceMat, axis=0)
    forceStd = _np.std(forceMat, axis=0)

    xNB, yNB, stdNB = _np.zeros(nNB, dtype=object), _np.zeros(nNB, dtype=object), _np.zeros(nNB, dtype=object)
    xB, yB, stdB = _np.zeros(nB, dtype=object), _np.zeros(nB, dtype=object), _np.zeros(nB, dtype=object)
    xA, yA, stdA = _np.zeros(nA, dtype=object), _np.zeros(nA, dtype=object), _np.zeros(nA, dtype=object)
    xD, yD, stdD = _np.zeros(nD, dtype=object), _np.zeros(nD, dtype=object), _np.zeros(nD, dtype=object)
    print('\noutput: {}.force'.format(output))
    sys_force = _cTAB(output + '.force', 'w')
    for i in range(nNB):
        xNB[i] = NB_x[i]
        c0, c1 = NB_wgt_ind[i]
        yNB[i] = forceAve[c0:c1]
        stdNB[i] = forceStd[c0:c1]
    if exclB:
        BondName = None
        BondType = None
    else:
        BondName = sysTop.BONDtypeSetName
        BondType = sysTop.BONDtypeSet
        for i in range(nB):
            xB[i] = B_x[i]
            c0, c1 = B_wgt_ind[i]
            yB[i] = forceAve[c0:c1]
            stdB[i] = forceStd[c0:c1]
    if exclA:
        AngleName = None
        AngleType = None
    else:
        AngleName = sysTop.ANGLEtypeSetName
        AngleType = sysTop.ANGLEtypeSet
        for i in range(nA):
                xA[i] = A_x[i]
                c0, c1 = A_wgt_ind[i]
                yA[i] = forceAve[c0:c1]
                stdA[i] = forceStd[c0:c1]
    if exclD:
        DihedralName = None
        DihedralType = None
    else:
        DihedralName = sysTop.DIHEDRALtypeSetName
        DihedralType = sysTop.DIHEDRALtypeSet
        for i in range(nD):
            xD[i] = D_x[i]
            c0, c1 = D_wgt_ind[i]
            yD[i] = forceAve[c0:c1]
            stdD[i] = forceStd[c0:c1]
    sys_force.setattr(NonBondType=sysTop.NonBONDED_Set, NonBondX=xNB, NonBondY=yNB, NonBondStd=stdNB,
        BondName=BondName, BondType=BondType, BondX=xB, BondY=yB, BondStd=stdB,
        AngleName=AngleName, AngleType=AngleType, AngleX=xA, AngleY=yA, AngleStd=stdA,
        DihedralName=DihedralName, DihedralType=DihedralType, DihedralX=xD, DihedralY=yD, DihedralStd=stdD)
    sys_force.write()
...
def genGMXexclusion(mapFile, top, out=None, bond=True, angle=True, dihedral=True):
    if out is None:
        if mapFile.endswith('.map'):
            out = mapFile[0:-4] + '.excl'
    else:
        if not out.endswith('.excl'):
            out += '.excl'
    mapping = _cMAP(mapFile)
    systop = _cTOP(top)
    bonded = _np.zeros(len(mapping.MOL), dtype=object)
    for n in range(len(systop.MOL)):
        bond_mol = _np.zeros([0, 2], dtype=int)
        if len(systop.BONDtype[n]) > 0 and bond:
            new = _np.concatenate(systop.BONDtypeIdx[n], axis=0)
            bond_mol = _np.append(bond_mol, new, axis=0)
        if len(systop.ANGLEtype[n]) > 0 and angle:
            new = _np.concatenate(systop.ANGLEtypeIdx[n], axis=0)[:, [0, 2]]
            bond_mol = _np.append(bond_mol, new, axis=0)
        if len(systop.DIHEDRALtype[n] > 0) and dihedral:
            new = _np.concatenate(systop.DIHEDRALtypeIdx[n], axis=0)[:, [0, 3]]
            bond_mol = _np.append(bond_mol, new, axis=0)
        bonded[n] = bond_mol
    exclusion = _np.zeros(len(mapping.MOL), dtype=object)
    for n in range(len(systop.MOL)):
        excl_mol = _np.zeros(mapping.nATOM[n], dtype=object)
        for i in range(mapping.nATOM[n]):
            l = [i]
            for j in range(i + 1, mapping.nATOM[n]):
                for k in range(len(mapping.BEADmap[n])):
                    if i in mapping.BEADmap[n][k]:
                        beadI = k
                    if j in mapping.BEADmap[n][k]:
                        beadJ = k
                cond1 = _np.all(bonded[n] == [beadI, beadJ], axis=1)
                cond2 = _np.all(bonded[n] == [beadJ, beadI], axis=1)
                if beadI == beadJ:
                    l.append(j)
                elif _np.any(cond1):
                    l.append(j)
                    ...
                    l.append(j)
            excl_mol[i] = _np.array(l, dtype=int)
        exclusion[n] = excl_mol + 1

    print('output : {}'.format(out))
    with open(out, 'w') as fid:
        for n in range(len(systop.MOL)):
            fid.write('MOL = {}\nNMOL = {}\n\n[ exclusions ]\n'.format(systop.MOL[n], systop.nMOL[n]))
            for i in range(mapping.nATOM[n]):
                if len(exclusion[n][i]) > 1:
                    for j in range(len(exclusion[n][i])):
                        fid.write('{} '.format(exclusion[n][i][j]))
                    fid.write('\n')
            if len(systop.MOL) > 1:
                fid.write('\n##########\n##########')
...
def ForceMatching(setFile, verbose=True):
        ...
        for n in range(len(tList)):
            ...
            if len(boxMat) == 15:
                ...
                lx, ly, lz, xy, xz, yz = boxMat[0], boxMat[1], boxMat[2], \
                                         boxMat[3], boxMat[4], boxMat[5]
            for i in range(len(sysTop.NonBONDED_SetIdx)):
                indA = _np.copy(sysTop.NonBONDED_SetIdx[i][:, 0])
                indB = _np.copy(sysTop.NonBONDED_SetIdx[i][:, 1])
                xA, yA, zA = tList[n]['xs'][indA], tList[n]['ys'][indA], tList[n]['zs'][indA]
                xB, yB, zB = tList[n]['xs'][indB], tList[n]['ys'][indB], tList[n]['zs'][indB]
                dx = xA - xB - _np.rint(xA - xB)
                dy = yA - yB - _np.rint(yA - yB)
                ...
                R = _np.sqrt(dX ** 2 + dY ** 2 + dZ ** 2)
                ind_cutoff = R <= cutoff
                indA = indA[ind_cutoff]
                indB = indB[ind_cutoff]
                dX = dX[ind_cutoff]
                dY = dY[ind_cutoff]
                dZ = dZ[ind_cutoff]
                R = R[ind_cutoff]
    ...
    nB = len(sysTop.BONDtypeSet)
    if not exclB:
        if len(sys_hLog.BondType) != nB:
            raise ValueError('ERROR in Bond types: "{}" and "" files are not compatible'.format(hLog, top))
        B_x = _np.zeros(nB, dtype=object)
        B_bw = _np.zeros(nB, dtype=float)
        B_wgt_ind = _np.zeros([nB, 2], dtype=int)
        for i in range(nB):
    ...
    nA = len(sysTop.ANGLEtypeSet)
    if not exclA:
        if len(sys_hLog.AngleType) != nA:
            raise ValueError('ERROR in Angle types: "{}" and "" files are not compatible.'.format(hLog, top))
        A_x = _np.zeros(nA, dtype=object)
        A_bw = _np.zeros(nA, dtype=float)
        A_wgt_ind = _np.zeros([nA, 2], dtype=int)
        for i in range(nA):
    ...
    nD = len(sysTop.DIHEDRALtypeSet)
    if not exclD:
        if len(sys_hLog.DihedralType) != nD:
            raise ValueError('ERROR in Dihedral types: "{}" and "" files are not compatible.'.format(hLog, top))
        D_x = _np.zeros(nD, dtype=object)
        D_bw = _np.zeros(nD, dtype=float)
        D_wgt_ind = _np.zeros([nD, 2], dtype=int)
        for i in range(nD):
